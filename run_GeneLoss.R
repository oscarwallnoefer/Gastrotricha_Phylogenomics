...
macrodasyida <- c( "Macrodasys_meristocytalis", "Anandrodasys_agadasys", "Crasiella_sp", 
                   "Thaidasys_tongiorgii", "Redudasys_fornerise", "Paradasys_sp", 
                   "Turbanella_ambronensis", "Urodasys_acanthostylis", "Mesodasys_laticaudatus",
                   "Paraturbanella_pallida", "Pleurodasys_incomptus", "Urodasys_bifidostylis",
                   "Neodasys_sp" ) 
chaetonotida <- c( "Diuronotus_aspetos", "Musellifer_sp", "Heteroxenotrichula_squamosa", 
                   "Xenotrichula_intermedia", "Musellifer_delamarei") 
oiorpata <- c( "Lepidodermella_sp", "Litigonotus_ghinii", "Aspidiophorus_tentaculatus", 
               "Chaetonotus_apolemmus", "Chaetonotus_sp", "Lepidodermella_squamata",
               "Lepidochaetus_zelinkai", "Aspidiophorus_sp", "Setopus_sp", "Stylochaeta_sp",
...
# nb: transposition. Use busco_num <- as.matrix(busco_mat) if you prefer
busco_num <- t(as.matrix(busco_mat))
svg("NEW_BUSCO_heatmap.svg", width = 10, height = 12)

colors <- colorRampPalette(c("darkred", "white"))(2)
